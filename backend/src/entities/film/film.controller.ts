...
export const syncFilmsDataToDB = async () => {
    try {
      let page = 1;
      let response = await fetch(`https://swapi.dev/api/films/?page=${page}&format=json`);
      ...
      while (response.ok) {
        ...
        for (const film of films) {
          ...
          const existingFilm = await filmModel.findOne({ episode_id: film.episode_id });
          ...
          if (!existingFilm) {
            ...
            console.log(`Film ${film.episode_id} already exists in the database`);
          }
        }
  
        if (data.next) {
          page++;
          response = await fetch(`https://swapi.dev/api/films/?page=${page}&format=json`);
        } else {
          break;
        }
      }

class Starship {
  ...
  async getStarships(limit: number, page: number = 1, name: string = "") {
    try {
      ...
      const response = await fetch(
        `https://digichanges-challenge-backend.onrender.com/api/v1/starships?limit=${limit}&offset=${offset}&name=${name}`
      );
  ...
  async getStarshipByName(name: string) {
    try {
      const response = await fetch(
        `https://digichanges-challenge-backend.onrender.com/api/v1/starships/${name}`
      );
      const data = await response.json();
      return data;
    } catch (error) {
